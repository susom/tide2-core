...
- **Token-accurate windowing**: The GPU actor covers **every token of every chunk**. Any chunk that tokenizes past the model's per-window budget (`model_max_length` minus special tokens) is sliced into overlapping ≤-budget token windows instead of being truncated — the earlier truncating path silently dropped the tail of token-dense chunks (real clinical text runs ~2.2–3.2 chars/token, so a 2048-char chunk can be ~770 tokens and lose ~30% of its content), so PHI there was never detected or redacted. Window predictions are merged back per chunk; overlap-region duplicates are removed by the existing downstream dedup (BIO raw-token tuples + reassembly IoU).
- **Adaptive GPU batching**: Auto-computes batch size from actual free GPU memory (`torch.cuda.mem_get_info`), capped to a modest first-attempt ceiling; windows are bucketed into length-homogeneous forward batches sized from exact token counts, so padding is near-uniform and per-forward memory is predictable (VRAM-aware budgets; override via `--short-seq-budget`).
- **OOM recovery**: On CUDA out-of-memory the GPU actor shrinks the window-batch and retries the forward over the **same** already-tokenized windows (never re-tokenizing) — a single owner of sub-batching, so no already-computed results are discarded. Recovery releases the failed forward's GPU tensors at the source so `empty_cache()` can actually reclaim between attempts and the retry converges instead of exhausting VRAM. The GPU allocator is also configured with `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` (via Ray `runtime_env`) as fragmentation headroom.
