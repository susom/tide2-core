...
- **In-actor length bucketing**: Before each forward the GPU actor orders its texts by length and partitions them into length-homogeneous groups, each sized from its own longest member, so padding is to a near-uniform length instead of the whole batch's max. This removes most padding waste on mixed short/long batches (measured 2–6× on such batches) and makes per-forward memory predictable. Bucketing is entirely in-actor (no Ray Data shuffle/sort, so no small-box deadlock) and reassembles outputs into the original input order.
- **Tokenize/forward overlap** (experimental, `--tokenize-overlap`, off by default): the GPU actor can tokenize the next length-bucket group on a background thread while the current group's forward runs (in-actor double-buffering), so the GPU idles less waiting on CPU tokenization. Whether it wins depends on the tokenize:forward ratio for your workload — measure with `dev/transformer_throughput_harness.py` before enabling.
